...
#     # Примеры времени
#     times = [
#         "08:00", "12:00", "15:00", "18:00", "20:00", "22:00"
#     ]
#     for time in times:
...
#
# # # Функция для получения общего количества событий
# def get_total_events(user_id: int):
#     cursor.execute('''
#     SELECT COUNT(*) FROM events WHERE user_id = ?
#     ''', (user_id,))
...
#
# # Функция для расчета следующей даты напоминания
# def calculate_next_reminder(remind_datetime: str, repeat_interval: str):
#     remind_datetime = datetime.strptime(remind_datetime, "%Y-%m-%d %H:%M")
...
#             # Если дня нет (например, 31 февраля), берем последний день месяца
#             if month == 2:
#                 day = 28 if not is_leap_year(year) else 29
#             else:
...
#         await message.answer("Привет, админ! Выберите действие:", reply_markup=admin_keyboard)
#     else:
#         await message.answer("Привет! Я бот для напоминаний о важных событиях. Используй кнопки ниже.", reply_markup=keyboard)
#
# # # Константы для пагинации
# EVENTS_PER_PAGE = 5  # Количество событий на одной странице
#
...
#     INSERT INTO events (user_id, event_name, remind_datetime, repeat_interval, category)
#     VALUES (?, ?, ?, ?, ?)
#     ''', (message.from_user.id, data['event_name'], data['remind_datetime'], data['repeat_interval'], category))
#     conn.commit()
#
#     await state.clear()
#     await message.answer("Событие успешно добавлено!", reply_markup=keyboard)
#
# # Шаг 5: Добавление новой категории
# @dp.message(Form.add_category)
# async def process_add_category(message: types.Message, state: FSMContext):
...
#
#     await state.clear()
#     await message.answer(f"Категория '{category_name}' успешно добавлена, и событие сохранено!", reply_markup=keyboard)
#
# # Обработка кнопки "Мои события"
# @dp.message(F.text == "Мои события")
...
#     ORDER BY remind_datetime ASC
#     LIMIT ? OFFSET ?
#     ''', (message.from_user.id, EVENTS_PER_PAGE, page * EVENTS_PER_PAGE))
...
#         response += f"🆔 {event[0]} — {event[1]} (напомнить: {event[2]}, повтор: {repeat_interval}, категория: {category})\n"
#
#     total_events = get_total_events(message.from_user.id)
#     total_pages = (total_events + EVENTS_PER_PAGE - 1) // EVENTS_PER_PAGE
#
#     # Добавляем кнопки пагинации
#     builder = InlineKeyboardBuilder()
...
#     event_id = message.text
#
#     # Проверка, существует ли событие с таким ID
#     cursor.execute('SELECT id FROM events WHERE id = ? AND user_id = ?', (event_id, message.from_user.id))
...
# @dp.message(F.text == "Отмена")
# async def cancel_disable(message: types.Message):
#     await message.answer("Отключение отменено.", reply_markup=keyboard)
#
# # Функция для проверки напоминаний
# async def check_reminders():
#     while True:
#         now = datetime.now().strftime("%Y-%m-%d %H:%M")
#         cursor.execute('SELECT id, user_id, event_name, remind_datetime, repeat_interval FROM events WHERE remind_datetime = ?', (now,))
...
# @dp.message(F.text == "Выйти из админки", F.from_user.id == ADMIN_ID)
# async def exit_admin_panel(message: types.Message):
#     await message.answer("Вы вышли из админки.", reply_markup=keyboard)
#
# # Запуск бота
# async def main():
#     loop = asyncio.get_event_loop()
